...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen bg-[#07090C] text-[#F1F5F9] flex">
      {/* Sidebar */}
      <aside className="w-[220px] shrink-0 bg-[#05070A] border-r border-white/[0.06] flex flex-col sticky top-0 h-screen">
        {/* Brand */}
        <div className="px-6 pt-7 pb-4 shrink-0">
          <Logo className="h-6" />
        </div>

        <div className="px-4 py-4 border-b border-white/[0.06] mx-2 mb-2">
          <div className="flex items-center gap-2 px-2">
            <div className="w-7 h-7 rounded-md bg-red/[0.15] border border-red/30 flex items-center justify-center">
            ...
            <span className="font-mono text-[14px] font-semibold tracking-tight">
              <span className="text-[#F1F5F9]">Admin</span>
            </span>
          </div>
        </div>
        ...
        <nav className="flex-1 px-3 py-4 flex flex-col gap-0.5 overflow-y-auto">
          {navItems.map(item => (
            <Link
              ...
              className={cn(
                ...
                isActive(item.href)
                  ? 'bg-sd-admin-active-bg text-sd-admin-active-ink [&_svg]:text-red'
                  : 'text-[#94A3B8] [&_svg]:text-[#64748B] hover:bg-white/[0.04] hover:text-[#CBD5E1]'
              )}
            >
              {item.icon}
              {item.label}
          ...
          <div className="mt-auto pt-4 border-t border-white/[0.06] flex flex-col gap-0.5">
            <Link
              href="/monitors"
              className="flex items-center gap-2.5 px-3 py-2 rounded-lg text-[13px] font-medium text-[#64748B] hover:bg-white/[0.04] hover:text-[#CBD5E1] transition-colors"
            >
            ...
            <button
              ...
              className="flex items-center gap-2.5 px-3 py-2 rounded-lg text-[13px] font-medium text-[#64748B] hover:bg-red/[0.1] hover:text-red transition-colors w-full"
            >
              <svg viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth="1.7" className="w-4 h-4">
      ...
      <div className="flex-1 flex flex-col min-w-0 bg-[#07090C]">
        ...
        {/* Topbar */}
        <header className="h-14 flex items-center justify-between px-6 bg-[#0F172A] border-b-2 border-sd-admin-border shrink-0 sticky top-0 z-30">
          <span className="font-mono text-[13px] text-[#64748B]">
            SonarDiff <span className="text-[#F1F5F9] font-semibold">Admin</span>
          </span>
          <div className="flex items-center gap-3">
            ...
            <button
              ...
              className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-white/[0.08] text-[#64748B] text-[12.5px] hover:bg-white/[0.04] hover:text-[#CBD5E1] transition-colors"
            >
        ...
        {/* Page content */}
        <main className="flex-1 overflow-y-auto p-8 bg-[#07090C]">
