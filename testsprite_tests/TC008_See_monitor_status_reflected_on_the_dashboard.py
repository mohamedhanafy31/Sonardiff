...
async def run_test():
    ...
    try:
        ...
        # -> Fill the email and password fields again and submit the sign-in form, then wait for the app to load to confirm login.
        # email input name="email"
        elem = page.locator("xpath=/html/body/div[2]/div/main/div/form/div/input").nth(0)
        await elem.wait_for(state="visible", timeout=10000)
        await elem.fill("mohamedhanafy3172003@gmail.com")
        ...
        # -> Fill the email and password fields with the provided credentials and submit the sign-in form to sign in, then observe the resulting page state.
        # email input name="email"
        elem = page.locator("xpath=/html/body/div[2]/div/main/div/form/div/input").nth(0)
        await elem.wait_for(state="visible", timeout=10000)
        await elem.fill("mohamedhanafy3172003@gmail.com")
        ...
        # -> Fill the email and password fields and submit the sign-in form to sign in (then wait for the SPA to load).
        # email input name="email"
        elem = page.locator("xpath=/html/body/div[2]/div/main/div/form/div/input").nth(0)
        await elem.wait_for(state="visible", timeout=10000)
        await elem.fill("mohamedhanafy3172003@gmail.com")
        ...
        # -> Open the monitor creation form by clicking the 'Add monitor' button on the Dashboard.
        # link "Add monitor"
        elem = page.locator("xpath=/html/body/div[2]/main/div/div/div/div/a").nth(0)
        await elem.wait_for(state="visible", timeout=10000)
        await elem.click()
        
        # -> Fill the URL and Monitor name fields, then click 'Continue — pick target' to proceed to the pick target step.
        # url input name="url"
        ...
        await elem.wait_for(state="visible", timeout=10000)
        await elem.fill("Example.com homepage - test monitor")
        ...
        # -> Advance from 'Pick target' to the 'Cadence & alerts' step by clicking Continue, then proceed to create the monitor.
        # button "Continue →"
        elem = page.locator("xpath=/html/body/div[2]/main/div/div/div/form/div/div[2]/div[6]/button[2]").nth(0)
        await elem.wait_for(state="visible", timeout=10000)
        await elem.click()
        
        # -> Click the 'Save monitor →' button to create the monitor, wait for the page to update, then verify the new monitor appears and that dashboard stats are displayed.
        # button "Save monitor →"
        elem = page.locator("xpath=/html/body/div[2]/main/div/div/div/form/div/div[2]/div[6]/button[2]").nth(0)
        await elem.wait_for(state="visible", timeout=10000)
        await elem.click()
